# -*- coding: utf-8 -*-
"""
Created on Tue Jan  7 17:53:38 2020
...
def rel_point(x):
    ...
    per = np.percentile(x,[0,25,50,75,100])
    Q1 = per[1]
    Q2 = per[2]
    Q3 = per[3]
...
def thanks_2019_vol2():
    ...
    ##### 새롭게 만든 함수를 적용 ##########
    m['t_score'] = rel_point(m['rel_td'])#
    m['s_score'] = rel_point(m['rel_sd'])#
    ######################################
    ...
    ##### scale 작업을 추가 ######################
    tr_data.iloc[:,0] = scale(tr_data.iloc[:,0])#
    #############################################
    ...
    file = open('c:/movie_project/predict/thanks_result_V2.csv','wb')  
    pickle.dump(result_t,file)
    file.close()
    
    file = open('c:/movie_project/predict/thanks_result_V2.csv','rb')
